...
class CsvEcosystemDataSource(
    ...
    override fun getMenteeById(id: String): MenteeRow? =
        getMentees().find { it.menteeId == id }
    override fun getMenteesByTeamId(teamId: String): List<MenteeRow> =
        getMentees().filter { it.teamId == teamId }
    override fun getTeams(): List<TeamRow> = teamLines.map {
        val parts = it.split(",")
        TeamRow(parts[0].trim(), parts[1].trim(), parts[2].trim())
    }
    override fun getTeamById(teamId: String): TeamRow? =
        getTeams().find { it.teamId == teamId }
    ...
    override fun getProjectByTeamId(teamId: String): ProjectRow? =
        getProjects().find { it.teamId == teamId }
